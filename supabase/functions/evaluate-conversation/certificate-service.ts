...
export class EdgeCertificateAPI {
  ...
  constructor(supabaseUrl, supabaseKey){
    ...
    this.algodClient = new algosdk.Algodv2(Deno.env.get('ALGOD_TOKEN') || '', Deno.env.get('ALGOD_SERVER') || 'https://testnet-api.algonode.cloud', Deno.env.get('ALGOD_PORT') || 443);
  ...
  // ... existing methods (generateCertificateId, calculateTranscriptHash, calculateAchievementLevel) ...
  async createAlgorandTransaction(certificateData) {
    try {
      // Get student's Algorand address
      const { data: userProfile } = await this.supabase.from('user_profiles').select('algorand_address').eq('user_id', certificateData.studentId).single();
      ...
      const txn = algosdk.makePaymentTxnWithSuggestedParams(this.issuerAccount.addr, studentAddress, 0, undefined, note, params // suggestedParams
      );
      // Sign transaction
  ...
  async onExaminationCompletion(studentId, courseId, examinationResults) {
    try {
      console.log('Starting certificate issuance for:', {
        studentId,
        courseId
      });
      // Get course information
      const { data: course, error: courseError } = await this.supabase.from('course_configuration').select('topic, depth').eq('id', courseId).single();
      ...
      const metadata = {
        ...
        achievementLevel: this.calculateAchievementLevel(examinationResults.totalScore, examinationResults.maxPossibleScore)
      };
      ...
      const { data: savedCertificate, error: certificateError } = await this.supabase.from('certificates').insert({
        certificate_id: certificateData.certificateId,
        student_id: certificateData.studentId,
